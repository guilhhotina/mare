...
local captions={inspect='Explore a ilha',raise='Elevar terreno',lower='Baixar terreno',level='Nivelar terreno',pull='Distorcer a costa',remove='Remover construcao',build='Construir'}
...
local function save_game(silent)
    ...
    if not silent then notify(ok and 'Ilha salva. Pode voltar quando quiser.' or 'Nao foi possivel salvar neste dispositivo.',ok) end
...
local function title_draw(std)
    ...
    title(std,78,78,'MARE',96,C.paper,432)
...
local function play_draw(std)
    ...
    if S.tool=='build' then
        ...
        msg=good and ('OK construir  /  '..World.price(w,S.build_id,S.cx,S.cy)..' moedas  /  VOLTAR opcoes') or (reason..'  /  VOLTAR opcoes')
        color=good and C.muted or C.red
        if S.paint then
            msg=S.gesture and 'SETAS tracar    OK terminar    VOLTAR cancelar' or 'OK comecar via    VOLTAR opcoes'
            color=S.gesture and C.gold or C.muted
        end
    elseif S.tool=='inspect' and id>0 then
        msg=(w.linked[w.occ[i]] and 'Acesso viario' or 'Precisa de uma via ao lado')..'    OK criar    VOLTAR pausa'
    ...
    elseif S.tool~='inspect' then
        msg=S.tool=='remove' and 'OK remover e recuperar 75%    VOLTAR explorar' or ((S.tool=='pull' and 'OK segurar' or S.tool=='level' and 'OK copiar altura' or 'OK comecar')..'    Pincel '..brush_names[S.brush]..'    VOLTAR opcoes')
    end
    ...
    if S.tool=='inspect' and w.reward<5 then
        ...
        text(std,70,180,ready and 'VOLTAR > Diario para receber' or (floor(World.progress(w)*100)..'%  /  Diario na pausa'),22,C.muted,336)
...
local function toolbar_draw(std)
    ...
    if S.selection==4 then note=#S.world.undo>0 and ('Desfazer disponivel: '..#S.world.undo..' acoes. O gesto inteiro volta.') or 'Nada para desfazer ainda.' end
...
local function context_draw(std)
    ...
    if S.tool=='build' then
        ...
        if S.paint then names={'Trocar via','Concluir'};note='Cada tracado pode ser desfeito de uma vez.'
        else names={'Girar peca','Trocar peca','Concluir'};note=S.selection==1 and 'OK gira 90 graus e volta para a ilha.' or S.selection==2 and 'Escolha outra peca. A ilha fica como esta.' or 'Volte a explorar a ilha.' end
    else
...
local function catalog_draw(std)
    ...
    footer(std,S.tab_focus and 'ESQUERDA / DIREITA categorias    OK ver construcoes    VOLTAR' or 'SETAS escolher    CIMA categorias    OK colocar    VOLTAR')
end
local function pause_draw(std)
    frame(std,'Um respiro','Sua ilha fica guardada enquanto voce faz uma pausa.')
    local opts={'Continuar jogando','Salvar a ilha','Diario da ilha','Ajustes','Como jogar','Contemplar a ilha','Salvar e ir ao inicio'}
...
local function settings_draw(std)
    frame(std,'Ajustes','Deixe a ilha confortavel para voce.')
    local opts={'Som: '..(S.sound and 'ligado' or 'desligado'),'Movimento: '..(S.motion and 'ligado' or 'reduzido'),'Detalhes: '..(S.detail==1 and 'completos' or 'economicos'),'Zoom: '..S.zoom..'x','Luz: '..Lighting.names[S.light],'Concluir'}
    ...
    text(std,808,468,'Manha, por do sol',28,C.muted,380)
...
local help_pages={
    ...
    {'Fazer a ilha crescer','Casas e lojas precisam de uma via ao lado.','Geradores e usinas precisam de acesso viario.','Energia e servicos atendem a ilha inteira.','Pracas e servicos melhoram a felicidade.'},
    ...
    {'Pontes e praias','Pontes ocupam agua ou margens na altura 1.','Portos e quiosques precisam ficar junto da agua.','Rampas retas aceitam trilhas, passeios e ruas.','Abrigos de animais atraem visitas para a ilha.'}
}
...
local function help_draw(std)
    ...
    footer(std,'ESQUERDA / DIREITA  paginas     OK ou VOLTAR  fechar')
end
local function goals_draw(std)
    local w=S.world;frame(std,'Diario da ilha',w.free and 'MODO LIVRE  /  Sem limites para experimentar' or 'JORNADA  /  Cada conquista abre novas possibilidades')
    local g=min(5,w.reward+1);local goal=World.goals[g]
    box(std,82,180,654,180,C.line)
    text(std,104,200,w.reward>=5 and 'A ilha e toda sua.' or goal[1],32,C.gold)
    text(std,104,250,w.reward>=5 and 'Todas as conquistas foram completadas.' or goal[2],26)
    text(std,104,306,w.reward>=5 and 'Continue criando, sem um fim obrigatorio.' or ('Recompensa: '..goal[3]..' moedas'),23,C.muted)
    ...
    text(std,82,528,'Animais: '..w.animals..'    Pracas: '..w.parks..'    Lojas: '..w.shops,25,C.muted)
    text(std,795,190,'A CIDADE EM NUMEROS',21,C.gold)
    local rows={{'Receita por dia',w.revenue},{'Manutencao',w.upkeep},{'Saldo por dia',w.balance},{'Agua / pessoas',w.water..' / '..w.population},{'Saude / pessoas',w.health..' / '..w.population},{'Educacao / pessoas',w.school..' / '..w.population}}
...
local function new_draw(std)
    ...
    text(std,910,482,'40 construcoes',23,C.muted)
...
local function confirm_draw(std)
    frame(std,'Substituir esta ilha?','O arquivo '..S.slot..' ja tem uma ilha salva.')
...
local function load_draw(std)
    ...
    local n=1;while n<=3 do
        ...
        menu_button(std,84,191+(n-1)*127,1108,'Ilha '..n,w and ('Dia '..w.day..'  /  '..w.population..' moradores  /  '..(w.free and 'Modo livre' or 'Jornada')) or (#data>0 and 'Arquivo invalido. Escolha outra ilha.' or 'Este arquivo esta vazio'),S.selection==n);n=n+1
    end
...
local function start_game()
    ...
    open('play');save_game(true);notify('Bem-vindo a sua ilha! OK abre as ferramentas.',true)
end
...
local function act(key)
    ...
    if screen=='play' then
        ...
        elseif key=='a' then
            ...
            elseif S.tool=='pull' then
                ...
                if S.gesture then finish_edit(S.gesture,'Relevo e construcoes ajustados.');S.gesture=nil;S.stroke=nil
                else
    ...
    if screen=='tools' or screen=='terrain' or screen=='context' then
        ...
        elseif key=='a' then
            ...
            if screen=='tools' then
                ...
                else local ok=World.undo(S.world);notify(ok and 'Ultima acao desfeita.' or 'Nada para desfazer ainda.',ok);if ok then explore() end end
    ...
    elseif screen=='load' then
        local w=S.saved[n]
        if w then S.world=w;S.slot=n;S.cx=11;S.cy=13;S.ox=640;S.oy=375;S.zoom=S.preferred_zoom or 2;S.tool='inspect';S.elapsed=0;home_camera();open('play') else notify('Este arquivo nao tem uma ilha valida.',false) end
...
return {meta={title='Mare',author='Ilhas Studio',description='Um pedacinho de mundo, do seu jeito.',version='0.4.0',id='studio.ilhas.mare'},callbacks={init=init,loop=loop,draw=draw}}
