...
async function frames(n=1){for(let i=0;i<n;i++){time+=34;const f=rafs;rafs=[];for(const fn of f){const before=win.mare&&win.mare.metrics.rebuilds;const started=process.hrtime.bigint();fn(time);const ms=Number(process.hrtime.bigint()-started)/1e6;timings.push(ms);const changed=win.mare&&before!==win.mare.metrics.rebuilds;(changed?rebuilt:steady).push(ms);if(ms>=16)slowFrames.push({ms,rebuild:changed,screen:win.mare&&win.mare.inspect('state').split(',')[0]});}await new Promise(r=>setImmediate(r));}}
...
const worldData=()=>win.mare.inspect('save').split(',').slice(6).join(',');
...
(async()=>{
 ...
 await go(11,12);const roadBefore=worldData(),roadHistory=history(),cashBefore=state()[3];
 ...
 await key('Enter');await go(11,11);assert.equal(win.mare.inspect('level'),heightBefore,'Painting never resamples the destination');assert.notEqual(worldData(),levelBefore);capture('31-nivelar');
 ...
 await toolsMenu(2);await focus(1);await key('Enter');await go(7,11);
 const villageBefore=worldData(),homeHeight=+win.mare.inspect('save').split(',')[6+11*25+7];
 await key('Enter');await key('ArrowRight');assert.equal(+win.mare.inspect('save').split(',')[6+11*25+7],homeHeight+1,'Occupied home rises with ground');capture('30-fundacoes');
 await key('Enter');await exitTool();await toolsMenu(4);assert.equal(worldData(),villageBefore);
 await toolsMenu(2);await focus(2);await key('Enter');await go(7,11);await key('Enter');assert.equal(+win.mare.inspect('save').split(',')[6+11*25+7],homeHeight-1,'Occupied home descends');await key('Escape');assert.equal(worldData(),villageBefore);await exitTool();
