...
const Pixels=win.MarePixels;let cameraZoom;
win.MarePixels=function(){
 const pixels=Pixels.apply(null,arguments),begin=pixels.begin;
 pixels.begin=function(csv,cx,cy,zoom){const result=begin.apply(null,arguments);cameraZoom=zoom;return result;};
 return pixels;
};
...
(async()=>{
 ...
 const explorationZoom=cameraZoom;
 ...
 await key('Escape');assert.equal(cameraZoom,explorationZoom,'Leaving contemplation restores the exploration zoom after drawing at another distance');
 await focus(5);await key('Enter');capture('16-guia');for(let i=0;i<4;i++)await key('ArrowRight');capture('17-guia-pontes');await key('Escape');assert.equal(state()[5],'5','Help restores parent focus');await key('Escape');
